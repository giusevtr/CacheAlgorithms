...
#         self.Y.append(1.0*self.P / self.N)
        ...
        assert t1+b1 <= 2*self.N, 'Error: t1+b1 should not be bigger than self.N. t1+b1=%d+%d=%d' % (t1,b1,t1+b1)
        assert t1+t2+b1+b2 <= 2.5*self.N, 'Error: t1+t2+b1+b2 should not be bigger than 4.5*self.N. t1+t2+b1+b2=%d+%d+%d+%d=%d' % (t1,t2,b1,b2,t1+t2+b1+b2)
        
	print "2.5*N", 2.5*self.N     
...
#	assert self.P <= 10`	
        ...
        if page in self.T1  or page in self.T2:
	        ...
	        print "Deleting page from T1"
            if page in self.T2 :
                assert self.T2.delete(page), 'failed deleting page in T2'
		print "Deleting page from T2"
